...
export default function PostCard({ post, currentUserId, onLike, onRepost, onDelete, isOwn }: PostCardProps) {
      ...
      {loadingComments ? (
        ...
        <>
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-white font-bold text-sm">コメント</h4>
  ...
  return (
    <>
      {/* 外枠: コメント開いてる時はPC版で横並び */}
      <div className="border-b border-gray-700">
        <div className={showComments ? "lg:flex lg:flex-row" : ""}>
          {/* 投稿本体 */}
          <div className="p-4 hover:bg-gray-900/50 transition-colors flex-1 min-w-0">
          ...
          {/* PC: コメント欄を右に表示 */}
          {showComments && (
            <div className="hidden lg:block lg:w-80 lg:shrink-0 border-l border-gray-700 bg-gray-900/50">
              {commentPanelContent}
            </div>
          )}
        </div>
        ...
        {/* モバイル: コメント欄を下に表示 */}
        {showComments && (
          <div className="lg:hidden border-t border-gray-700 bg-gray-900/50">
            {commentPanelContent}
          </div>
        )}
      </div>
