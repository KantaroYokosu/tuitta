  ...
  const renderComment = (comment: Comment, isChild = false) => (
    <div key={comment.id} className={`flex gap-2 mb-3 group/comment ${isChild ? "ml-8" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {comment.user.id === currentUserId && (
            <button
              ...
              className="ml-auto text-gray-400 hover:text-red-500 transition-colors opacity-0 group-hover/comment:opacity-100"
            >
        ...
        <p className="text-gray-900 text-sm mt-0.5 whitespace-pre-wrap">{comment.content}</p>
        {!isChild && (
          <button
            onClick={() => setReplyTo({ id: comment.id, name: comment.user.name })}
            className="text-gray-400 hover:text-[#4BACC5] text-xs mt-1 transition-colors"
          >
            返信
          </button>
        )}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <h3 className="text-gray-900 font-bold">コメント</h3>
      ...
      <div className="p-4 border-t border-gray-200">
        {replyTo && (
          <div className="flex items-center gap-2 mb-2 text-xs text-gray-400">
            ...
            <button onClick={() => setReplyTo(null)} className="text-gray-400 hover:text-gray-900">✕</button>
          </div>
        )}
        <div className="flex gap-2">
          <textarea
            ...
            maxLength={140}
            rows={2}
            className="flex-1 bg-transparent border border-gray-300 rounded-xl px-3 py-2 text-gray-900 text-sm outline-none focus:border-[#4BACC5] resize-none"
          />
          <button
            onClick={submitComment}
            disabled={commentText.trim() === "" || submitting}
            className="bg-[#4BACC5] hover:bg-[#3a9ab3] disabled:opacity-50 text-white font-bold px-4 py-2 rounded-full text-sm transition-colors self-end"
          >
            返信
          </button>
        </div>
        <p className="text-gray-400 text-xs mt-1">Ctrl+Enter（Mac: ⌘+Enter）で送信</p>
      </div>
    </div>
  );
}
